...
data <- read.csv("C:/Users/Lenovo/Desktop/Studia/PSI/11/LOT_reviews.csv", stringsAsFactors = FALSE, encoding = "UTF-8")
...
# Ustawienia dla pierwszego słowa
target_word_1 <- "service"
cor_limit_1 <- 0.3 # Obniżono próg, aby pokazać szerszy kontekst
...
# 1. Wykres lizakowy dla "service"
ggplot(assoc_df_1, aes(x = score, y = reorder(word, score))) +
  geom_segment(aes(x = 0, xend = score, y = word, yend = word), color = "#74a9cf", size = 1.2) +
  geom_point(color = "#045a8d", size = 4) +
  geom_text(aes(label = round(score, 2)), hjust = -0.3, size = 3.5, color = "black") +
  scale_x_continuous(limits = c(0, max(assoc_df_1$score, 0.5) + 0.1), expand = expansion(mult = c(0, 0.2))) +
  theme_minimal(base_size = 12) +
  labs(
    title = paste0("Asocjacje z terminem: '", target_word_1, "'"),
    subtitle = paste0("Próg r ≥ ", cor_limit_1),
    x = "Korelacja Pearsona",
    y = "Słowo"
  )

# 2. Wykres lizakowy z natężeniem dla "service"
ggplot(assoc_df_1, aes(x = score, y = reorder(word, score), color = score)) +
  geom_segment(aes(x = 0, xend = score, y = word, yend = word), size = 1.2) +
  geom_point(size = 4) +
  geom_text(aes(label = round(score, 2)), hjust = -0.3, size = 3.5, color = "black") +
  scale_color_gradient(low = "#bdc9e1", high = "#045a8d") +
  theme_minimal(base_size = 12) +
  labs(
    title = paste0("Natężenie asocjacji dla: '", target_word_1, "'"),
    x = "Korelacja Pearsona",
    y = "Słowo",
    color = "Siła korelacji"
  )


#' ## Analiza dla słowa: "warsaw"


# Ustawienia dla drugiego słowa
target_word_2 <- "warsaw"
cor_limit_2 <- 0.3
...
# 1. Wykres lizakowy dla "warsaw"
ggplot(assoc_df_2, aes(x = score, y = reorder(word, score))) +
  geom_segment(aes(x = 0, xend = score, y = word, yend = word), color = "#feb24c", size = 1.2) +
  geom_point(color = "#f03b20", size = 4) +
  geom_text(aes(label = round(score, 2)), hjust = -0.3, size = 3.5, color = "black") +
  scale_x_continuous(limits = c(0, max(assoc_df_2$score, 0.5) + 0.1), expand = expansion(mult = c(0, 0.2))) +
  theme_minimal(base_size = 12) +
  labs(
    title = paste0("Asocjacje z terminem: '", target_word_2, "'"),
    subtitle = paste0("Próg r ≥ ", cor_limit_2),
    x = "Korelacja Pearsona",
    y = "Słowo"
  )

# 2. Wykres lizakowy z natężeniem dla "warsaw"
ggplot(assoc_df_2, aes(x = score, y = reorder(word, score), color = score)) +
  geom_segment(aes(x = 0, xend = score, y = word, yend = word), size = 1.2) +
  geom_point(size = 4) +
  geom_text(aes(label = round(score, 2)), hjust = -0.3, size = 3.5, color = "black") +
  ...
  theme_minimal(base_size = 12) +
  labs(
    title = paste0("Natężenie asocjacji dla: '", target_word_2, "'"),
    x = "Korelacja Pearsona",
    y = "Słowo",
    color = "Siła korelacji"
  )
